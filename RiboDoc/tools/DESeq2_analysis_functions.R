...
ma_plot <- function(deseq_results) {
  ...
  deseq_results_modified$padj[which(abs(deseq_results_modified$log2FoldChange) <= abs(Var_log2FC))] <-
    1
  plotMA(
    deseq_results_modified,
    alpha = Var_padj,
    ylab = "log2FC",
    colSig = rgb(1, 0, 0, 0.5)
  )
}
...
volcano_plot <- function(deseq_results, data_list) {
  myColors <-
    ifelse((
      deseq_results$padj < Var_padj &
        deseq_results$log2FoldChange > Var_log2FC
    ),
    rgb(1, 0, 0, 0.25) ,
    ifelse((
      deseq_results$padj < Var_padj &
  ...
  plot(
    deseq_results[, "log2FoldChange"],
    -log10(deseq_results[, "padj"]),
    pch = 20,
    cex = 1,
    col = myColors,
    xlab = "log2 FC",
    ylab = "-log10(padj)",
    main = paste0("Volcano plot - ", data_list$Cond[data_list$nb_col_condB +
                                                      1], " vs ", data_list$Cond[1])
  )
}
...
volcano_plot_zoom <- function(deseq_results, data_list) {
  ...
  plot(
    deseq_results[, "log2FoldChange"],
    -log10(Valeurs_Limite_Y),
    pch = Forme_Pixel_Y,
    col = myColors,
    xlab = "log2 FC",
    ylab = "-log10(padj)",
    ylim = c(0, -log10(limite)),
    main = paste0("Volcano plot Zoom - ", data_list$Cond[data_list$nb_col_condB +
                                                           1], " vs ", data_list$Cond[1])
  )
}
