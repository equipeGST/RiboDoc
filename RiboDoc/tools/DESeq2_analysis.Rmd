...
{
  DESeq2_folder <- paste0(local_path, "RESULTS/DESeq2_by_gene/")
} else {
  DESeq2_folder <- paste0(local_path, "RESULTS/DESeq2_by_transcript/")
}
...
```{r data , echo=FALSE}
expData = round(read.table(pathway_matrix, header = T, row.names = 1, check.names = FALSE, sep="\t", quote = ""))

names_list = read.table(pathway_names, header = T, row.names = 1, check.names = FALSE, sep = "\t", quote = "")
```
...
pairs(log10(expData_Sorted), 
      ...
      main = "Pairwise scatter plot and Spearman correlation coefficients"
      )
```
\
...
plotMA(resDESeq, alpha = Var_padj, ylab = "log2 FC", colSig = rgb(1,0,0,0.5), main = "MA-plot")
```
