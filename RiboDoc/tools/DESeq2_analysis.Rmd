...
{
  DESeq2_folder <- "DESeq2_by_gene"
} else {
  DESeq2_folder <- "DESeq2_by_transcript"
}
...
dir.create(paste0("/data/RESULTS/",DESeq2_folder,"/Images/"))
```

```{r setup, echo=FALSE, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
knitr::opts_chunk$set(include = TRUE, message = FALSE, warning = FALSE)
knitr::opts_chunk$set(fig.path = paste0("/data/RESULTS/",DESeq2_folder,"/Images/"), dev=c('png','tiff'))
```


```{r data , echo=FALSE}
pathway_matrix = paste0("/data/RESULTS/",DESeq2_folder,"/count_matrix.txt")
expData = round(read.table(pathway_matrix, header = T, row.names = 1, check.names = FALSE))

pathway_names = paste0("/data/RESULTS/",DESeq2_folder,"/transcript_list.txt")
...
if(trans_or_gene=="gene")
{
  ...
  write.table(expData, paste0("/data/RESULTS/",DESeq2_folder,"/count_matrix_mRNA_by_gene.txt"), sep = "\t")
}
```
...
write.table(allGenes,
            file = paste0("/data/RESULTS/",DESeq2_folder,"/complete.txt"),
            quote = F, sep = "\t", row.names = F)


write.table(inducedGenes,
            file = paste0("/data/RESULTS/",DESeq2_folder,"/up.txt"),
            quote = F, sep = "\t", row.names = F)

write.table(repressedGenes,
            file = paste0("/data/RESULTS/",DESeq2_folder,"/down.txt"),
            quote = F, sep = "\t", row.names = F)
```
\
**OUTPUT FILES** :  
All the output files can be found in the directory “RESULTS/DESeq2_by_gene/” or " RESULTS/DESeq2_by_transcript/”  
`r paste0("RESULTS/",DESeq2_folder,"/complete.txt")`  
`r paste0("RESULTS/",DESeq2_folder,"/up.txt")  `  
`r paste0("RESULTS/",DESeq2_folder,"/down.txt")  `   
\
