...
paths_list <- DESeq2_folder_paths(working_dir, seq_type, feature)
```
\
...
knitr::opts_chunk$set(fig.path = paste0(paths_list$DESeq2_gene, "Images/", refCond, "_vs_", other_cond, "_"), dev = c('png','tiff'))
```
...
expData_gene_all <- expData_gene_all %>%
...
\
`r nrow(expData_gene_all)` features are compared in this study, with 2 conditions ; `r refCond` and `rother_cond`. </div>
...
make_PCA(expData_sorted_gene$expData_Sorted, 2, c(other_cond, refCond))
```
\
...
make_PCA(expData_sorted_gene$expData_Sorted, 3, c(other_cond, refCond))
```
\
...
library_size_barplot(dds_object_gene, expData_sorted_gene, other_cond, refCond)
```
...
library_size_boxplot(dds_object_gene, expData_sorted_gene, other_cond, refCond)
```
...
resDESeq_gene <- dispersion_estimation(dds_object_gene, other_cond, refCond)
```
...
frequency_histogram(resDESeq_gene, other_cond, refCond)
```
\
...
pval_distribution(resDESeq_gene, other_cond, refCond)
```
...
padj_distribution(resDESeq_gene, other_cond, refCond)
```
\
...
ma_plot(resDESeq_gene, expData_sorted_gene, other_cond, refCond)
```
...
volcano_plot(resDESeq_gene, expData_sorted_gene, other_cond, refCond)
```
...
volcano_plot_zoom(resDESeq_gene, expData_sorted_gene, other_cond, refCond)
```
\
...
write_DE_tables(tables_gene, "gene", refCond, other_cond)
```
\
**OUTPUT FILES** :
