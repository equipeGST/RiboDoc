...
local_path        <- params$working_dir
deseq2_functions  <- params$path_function
Var_log2FC        <- params$log2fc
Var_padj          <- params$padj
refCond           <- params$refCond
other_cond        <- params$other_cond
seq_type          <- params$seq_type
feature           <- params$feature
```

```{r Load functions and determine paths, echo=FALSE, include=FALSE}
source(paste0(deseq2_functions))
paths_list <- DESeq2_folder_paths(local_path, seq_type, feature)
```
...
knitr::opts_chunk$set(fig.path = paste0(paths_list$DESeq2_transcript, "Images/", refCond, "_vs_", other_cond, "_"), dev = c('png','tiff'))
```
...
expData_transcript_all <- expData_transcript_all %>%
      select(contains(refCond) | contains(other_cond))
```
...
\
`r nrow(expData_transcript_all)` features are compared in this study, with 2 conditions ; `r refCond` and `other_cond`. </div>
...
make_PCA(expData_sorted_transcript$expData_Sorted, 2, c(other_cond, refCond))
```
\
...
make_PCA(expData_sorted_transcript$expData_Sorted, 3, c(other_cond, refCond))
```
\
...
library_size_barplot(dds_object_transcript, expData_sorted_transcript, other_cond, refCond)
```
...
library_size_boxplot(dds_object_transcript, expData_sorted_transcript, other_cond, refCond)
```
...
resDESeq_transcript <- dispersion_estimation(dds_object_transcript, other_cond, refCond)
```
...
frequency_histogram(resDESeq_transcript, other_cond, refCond)
```
\
...
pval_distribution(resDESeq_transcript, other_cond, refCond)
```
...
padj_distribution(resDESeq_transcript, other_cond, refCond)
```
\
...
ma_plot(resDESeq_transcript, expData_sorted_transcript, other_cond, refCond)
```
...
volcano_plot(resDESeq_transcript, expData_sorted_transcript, other_cond, refCond)
```
...
volcano_plot_zoom(resDESeq_transcript, expData_sorted_transcript, other_cond, refCond)
```
\

```{r Final tables creation by transcript, include = FALSE}
table_transcript <- tables_creation(
      dds_object = dds_object_transcript,
...
write_DE_tables(table_transcript, "transcript", refCond, other_cond)
```
\
**OUTPUT FILES** :  
